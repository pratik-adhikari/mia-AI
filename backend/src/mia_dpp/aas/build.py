...
def build_dpp(
    ...
    *,
    ...
    global_seed = {
        ...
        "templates": [
            templates.load(key).release.model_dump(mode="json")
    ...
    primary = artifacts[0]
    shell = copy.deepcopy(
        cast(dict[str, Any], primary.environment["assetAdministrationShells"][0])
    )
    ...
    aggregate_gap_report = GapReport(
        ...
        blocks_deployment=any(
            ...
            for gap in aggregate_gaps
        ),
    )
    deployable = (
        aggregate_validation.valid
        and not aggregate_gap_report.blocks_deployment
    )
...
def _select_evidence(
    ...
    *,
    ...
    if not supplied:
        generated = tuple(
            _evidence(mapping, product_name, acquired_at)
            for mapping in mappings
        )
