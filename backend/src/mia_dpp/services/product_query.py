...
class ProductQueryService:
    ...
    def work_status(self, thread_id: str, *, user_id: str) -> WorkStatusView:
        ...
        return WorkStatusView(
            ...
            human_review_pending=(snapshot.human_review_pending if snapshot is not None else False),
            unresolved_required_count=(
                len(snapshot.unresolved_required_ids) if snapshot is not None else 0
            ),
            last_error=((snapshot.last_error if snapshot is not None else None) or run.error),
    ...
    def search_evidence(
        ...
        *,
        ...
        if artifact is None:
            return ()
        package = ProductKnowledgePackage.model_validate_json(self._artifacts.get(artifact))

        tokens = tuple(
            token for token in re.findall(r"[a-z0-9]+", query.casefold()) if len(token) > 1
        )
