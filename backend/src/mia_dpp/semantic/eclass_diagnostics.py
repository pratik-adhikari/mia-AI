...
def _diagnose_result(result: EclassEvidenceResolution) -> EclassEvidenceDiagnostics:
    ...
    return EclassEvidenceDiagnostics(
        ...
        minimum_selected_probability=min(
            item.diagnostics.selected_probability for item in scoped
        ),
        minimum_margin=min(item.diagnostics.margin for item in scoped),
        maximum_runner_up_ratio=max(
            item.diagnostics.runner_up_ratio for item in scoped
        ),
        maximum_normalized_entropy=max(
            item.diagnostics.normalized_entropy for item in scoped
        ),
        all_choices_are_argmax=all(
            item.diagnostics.choice_is_argmax for item in scoped
        ),
        unresolved_scope_count=sum(
            item.diagnostics.selected_choice == UNRESOLVED_ECLASS
            for item in scoped
        ),
    )
...
def _strong_conflicting_choices(
    ...
    strong_choices = {
        ...
        if (
            item.diagnostics.selected_probability
            >= settings.alarm_strong_route_probability
...
def classify_eclass_diagnostics(
    ...
    auto = (
        ...
        >= settings.auto_min_selected_probability
        ...
        and diagnostics.maximum_normalized_entropy
        <= settings.auto_max_normalized_entropy
    ...
    optional = (
        diagnostics.minimum_selected_probability
        >= settings.optional_min_selected_probability
        and diagnostics.minimum_margin >= settings.optional_min_margin
        and diagnostics.maximum_runner_up_ratio
        <= settings.optional_max_runner_up_ratio
        and diagnostics.maximum_normalized_entropy
        <= settings.optional_max_normalized_entropy
    ...
    if (
        diagnostics.maximum_normalized_entropy
        > settings.optional_max_normalized_entropy
    ):
...
def eclass_policy_decisions(
    ...
    decisions = (
        classify_eclass_diagnostics(item, settings)
