...
def choice_diagnostics(
    *,
    ...
    runner_up_ratio = runner_up_probability / top_probability if top_probability > 0.0 else 1.0
...
def diagnose_trace(trace: IdtaRoutingTrace) -> RouteDiagnostics:
    ...
    if not decisions:
        ...
        minimum_selected_probability = min(item.selected_probability for item in decisions)
        ...
        maximum_normalized_entropy = max(item.normalized_entropy for item in decisions)
    ...
    for evidence_id, routes_list in sorted(grouped.items()):
        ...
        diagnostics.append(
            EvidenceRoutingDiagnostics(
                ...
                maximum_runner_up_ratio=max(item.maximum_runner_up_ratio for item in routes),
                maximum_normalized_entropy=max(item.maximum_normalized_entropy for item in routes),
                all_choices_are_argmax=all(item.all_choices_are_argmax for item in routes),
