...
def _group_criterion(
    ...
    *,
    ...
    for identifier in group.evidence_ids[:max_members]:
        ...
        label = record.source_label or record.predicate
        members.append(f"{label}={record.value!s} @ {' / '.join(record.context_path) or '<root>'}")
    suffix = (
...
def _view_for(
    ...
    if len(matches) != 1:
        raise ValueError(f"expected one {scope.value} context view for evidence {evidence_id!r}")
    return matches[0]
...
async def jev_incremental_grouping(
    *,
    ...
    for record in package.evidence:
        ...
        criteria = {group.id: _group_criterion(group, records) for group in groups}
        criteria[NEW_GROUP] = (
        ...
        groups[group_index] = group.model_copy(
            update={
                ...
                "source_paths": tuple(dict.fromkeys((*group.source_paths, record.context_path))),
            }
        )
        assignments.append(
            GroupAssignment(
    ...
    for left_index, left in enumerate(evidence_ids):
        for right in evidence_ids[left_index + 1 :]:
            ...
            for run, assignments in maps:
                ...
                if left_assignment.group_id is None or right_assignment.group_id is None:
