...
def _review_id(cycle_seed: str, evidence_id: str) -> str:
    return "review-" + hashlib.sha256(f"{cycle_seed}\0{evidence_id}".encode()).hexdigest()[:24]
    ...
    *,
    ...
    return FieldMapping(
        ...
        assessment=MappingAssessment(
            ...
            reason=reason,
            uncertainties=(("Human confirmation is required.",) if review_required else ()),
        ),
        reasoning=reason,
...
def _replace_with(
    ...
    *,
    ...
    mapped = [item for item in mapping.mapped if item.evidence_id != record.id]
    ambiguous = [item for item in mapping.ambiguous if item.evidence_id != record.id]
    rejected = tuple(item for item in mapping.rejected if item.evidence_id != record.id)
    unmatched = [item for item in mapping.unmatched_evidence_ids if item != record.id]
    irrelevant = tuple(item for item in mapping.irrelevant_evidence_ids if item != record.id)
    rejected_ids = tuple(item for item in mapping.rejected_evidence_ids if item != record.id)
    outcomes = [item for item in mapping.outcomes if item.evidence_id != record.id]
...
def _fixed_review_rows(
    ...
    return tuple(
        ...
        if (
            ...
            or (item.mapping is not None and item.mapping.assessment.review_required)
        )
    )
...
def promote_open_properties(
    *,
    ...
    resolution_by_id = {item.evidence_id: item for item in eclass_resolution.results}
    diagnostics_by_id = {item.evidence_id: item for item in eclass_diagnostics.evidence}
        ...
        if requires_review and candidates:
            ...
            review_items.append(
                SemanticReviewItem(
                    ...
                    review_priority=(
                        "alarm"
                        if conflict or proposal.review_priority is DecisionPriority.ALARM
                        else "confirm"
                    ),
                    reason=(
