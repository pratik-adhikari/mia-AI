...
def _diagnostics(*routes: RouteDiagnostics) -> EvidenceRoutingDiagnostics:
    ...
    return EvidenceRoutingDiagnostics(
        ...
        minimum_selected_probability=min(route.minimum_selected_probability for route in routes),
        minimum_margin=min(route.minimum_margin for route in routes),
        maximum_runner_up_ratio=max(route.maximum_runner_up_ratio for route in routes),
        maximum_normalized_entropy=max(route.maximum_normalized_entropy for route in routes),
        all_choices_are_argmax=True,
