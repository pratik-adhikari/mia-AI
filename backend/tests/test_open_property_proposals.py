...
def test_same_semantic_slot_with_different_values_is_value_conflict() -> None:
    ...
    conflicts = [
        item for item in report.conflicts if item.kind is OpenPropertyConflictKind.VALUE_CONFLICT
    ]
...
def test_equivalent_normalized_values_do_not_conflict() -> None:
    ...
    assert not any(
        item.kind is OpenPropertyConflictKind.VALUE_CONFLICT for item in report.conflicts
    )
    assert any(
        item.kind is OpenPropertyConflictKind.REDUNDANT_DUPLICATE for item in report.conflicts
    )
...
def test_same_concept_in_different_component_contexts_get_distinct_area_bindings() -> None:
    ...
    proposals = {item.evidence_id: item for item in report.proposals}
    left = proposals["ev-1"]
