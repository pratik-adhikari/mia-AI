...
class _Decider:
    ...
    async def choose(
        ...
        *,
        ...
        choice = NO_ECLASS_MATCH if self.no_match else "0173-1#02-AAO677#001"
...
def test_eclass_resolution_only_processes_open_technical_properties() -> None:
    ...
    assert tuple(item.irdi for item in technical.verified_candidates) == ("0173-1#02-AAO677#001",)
    assert len(technical.decisions) == 2
    assert all(scoped.decision.choice == "0173-1#02-AAO677#001" for scoped in technical.decisions)
