...
def test_compiler_materializes_distinct_technical_property_area_instances() -> None:
    ...
    assert mappings[0].target.projection_identity != mappings[2].target.projection_identity
    ...
    for area in area_instances:
        ...
        values = {item["idShort"]: item.get("value") for item in area["value"]}
        property_sets.append(values)

    assert {tuple(sorted(values.items())) for values in property_sets} == {
